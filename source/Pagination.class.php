...
class Pagination{
	...
	var $numLinks		=  2;
	var $currentPage	=  0;
	var $firstLink   	= ' &lsaquo; Pierwszy';
	var $nextLink		= '&gt;';
	var $prevLink		= '&lt;';
	var $lastLink		= ' Ostatni &rsaquo;';
	...
	function createLinks(){ 
		...
		if ($numPages == 1){
			if ($this->showCount){
				$info = '<div class="pagination_none"> Łącznie : ' . $this->totalRows . " produktów </div>";
				return $info;
			}else{
				return '';
			}
		...
		if ($this->showCount){
		   ...
		   $info = 'Wyświetlaj od ' . ( $currentOffset + 1 ) . ' do ' ;
		   ...
		   $info .= ' z łącznie ' . $this->totalRows . ' produktów | ';
	...
	function getAJAXlink( $count, $text) {
		...
		return "<a href=\"javascript:void(0);\" " . $this->anchorClass . "onclick=\"    $.ajax({
        type: 'POST',
        url: '". $this->baseURL."',
